...
## 6. Accessing via EC2 Public IP
To access the application from the internet using your EC2's Public IP:

1.  **AWS Security Group**: Ensure your EC2 Security Group allows **Inbound TCP traffic** on ports `30007`, `30008`, and `30009`.
2.  **Port Forwarding**: Run the following command on your EC2 instance (keep it running):
    ```bash
    kubectl port-forward --address 0.0.0.0 service/collage-erp-service 30007:80
    ```
3.  **Browser**: Open `http://<EC2-PUBLIC-IP>:30007`.

*Note: For Prometheus/Grafana, use similar commands mapping 30008:9090 and 30009:3000 respectively.*
